...
module/      → 可插拔游戏模块，DDD 四层架构（internal/domain / application / infrastructure / interface）
  ...
  playerlife/ → 玩家连接生命周期（Grace Period、Actor Kill/Spawn、节点解绑）
...
- **玩家生命周期组件**：`module/playerlife`，负责断线 Grace Period、Actor Kill/Spawn 和节点解绑
...
- `docs/用户延迟登出设计.md` — 两阶段生命周期、playerlife.Manager
