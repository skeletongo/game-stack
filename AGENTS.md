...
module/      → 可插拔游戏模块，DDD 四层架构（internal/domain / application / infrastructure / interface）
  ...
  clean/     → 玩家延迟清理器（PlayerDoneCleaner + CleanablePlayer）
...
- **清理器独立模块**：`module/clean`，通过 `clean.Get().Register(adapter)` 注册清理回调
...
- `docs/用户延迟登出设计.md` — 两阶段清理、PlayerDoneCleaner / CleanablePlayer
