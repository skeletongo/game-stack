...
{
  "queries": ["get"],
  "commands": ["set_avatar", "add_exp", "add_gold", "deduct_gold", 
               "add_diamond", "deduct_diamond", "delete_player"]
}
```

`commands` 直接从 `cmdBus.CommandNames()` 获取。`queries` 通过通用 `/debug/query` 端点。
...
```json
→ {"module": "player", "cmd": "add_gold", "params": {"player_id": 12345, "amount": 999}}
← {"ok": true}
```

实现：`cmdBus.NewCommand(name)` 获取零值 → `json.Unmarshal(params, cmd)` → `cmdBus.Dispatch(ctx, cmd)`。命令通过已有的 `SetAvatarHandler` 等处理器执行，自动发布领域事件。
...
debug.Register[*domain.Account]("auth", repo, nil)  // auth 无 CommandBus
```

auth 的登录/注册处理器返回 `(*Result, error)`，不符合 `CommandHandler` 签名，没有注册到 CommandBus。传 `nil` 后 auth 只开放 query 和 patch，不开放 command。
...
```go
func Register[C Command](bus *CommandBus, name string, handler CommandHandler[C])
...
`Register` 存 handler + 零值工厂 `func() any { return new(C) }`。`NewCommand` 返回 `any`（`*Cmd` 零值指针）而非 `Command`，因 Go 泛型限制 `*C` 不可直接转为接口。
